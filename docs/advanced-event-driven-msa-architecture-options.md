...
Flink or Kafka Streams
  ...
  -> partial fill 집계
  -> InfiniteBuyV4 state transition
...
- `stock-purchase-service` 내부 또는 인접 모듈에서 체결 이벤트 dedup
- 주문별 partial fill 합산
...
- `InfiniteBuyV4`의 상태를 event log만으로 재구성한다.
...
4. `stock-search-service`와 `stock-purchase-service`를 event consumer/producer 중심으로 정렬한다.
