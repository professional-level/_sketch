...
- Run `docs/operations/github/verify-github-delivery.ps1 -CommitSha <git-sha>` or an equivalent check to verify the `Container Images` and `Operations Validation` workflows succeeded and the immutable GHCR image tags exist before rollout.
