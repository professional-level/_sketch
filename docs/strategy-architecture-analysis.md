...
- `stock-purchase-service`는 broker 제출 전 risk guard로 주문 단위 금액 한도, 종목별 주문 금액 한도, 활성 매수 주문 기준 계좌 pending exposure 한도, 하루 broker 제출 건수 한도, 동일 전략/종목/태그 중복 kill switch, 전략 prefix disable을 적용한다.
- 실제 체결 보유 수량/현금 기반 계좌 전체 exposure와 모의투자/실전투자 분리 정책은 별도 계좌/포지션 상태 모델이 필요하므로 아직 남아 있다.
...
16. 부분 완료: stock-purchase-service의 broker 제출 전 risk guard를 추가한다. 주문 단위/종목별 금액 한도, 활성 매수 주문 기준 계좌 pending exposure 한도, 하루 broker 제출 건수 한도, 중복 주문 kill switch, 전략 prefix disable은 적용됐고, 실제 체결 포지션/현금 기반 계좌 전체 exposure와 모의/실전 계좌 분리는 남아 있다.
