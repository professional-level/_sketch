...
- KIS alias 기반 cash bucket 분리와 risk guard 내부 기준 통화 환산은 추가됐지만, 실시간 FX source 연동과 계좌 현금 상태 이벤트 계약 확장은 아직 남아 있다.
...
- Risk guard cash checks prefer `orderableCashAmount` and fall back to the legacy `availableCashAmount` only for compatibility with older account snapshot providers. Order notional, pending buy notional, broker exposure, and orderable cash are normalized to the configured risk base currency with static `rates-to-base` configuration. Live FX sourcing and broader account cash/exposure modeling remain hardening work.
