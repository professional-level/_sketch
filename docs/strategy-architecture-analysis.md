...
- Domestic KIS cancel requests require `KRX_FWDG_ORD_ORGNO` and `ORGN_ODNO`; submitted KIS `KRX_FWDG_ORD_ORGNO` values are persisted on `order_intent_submission.branchOrderNumber` and reused when a later domestic cancel request omits the branch/order-org number. KIS all-zero original-order placeholders such as `0000000000` are ignored during broker history mapping so they are not treated as real linked broker order ids. Overseas cancel requests use `OVRS_EXCG_CD`, `PDNO`, and `ORGN_ODNO`. Modify/revise orders and production credential/live-market verification are still production-hardening gaps.
