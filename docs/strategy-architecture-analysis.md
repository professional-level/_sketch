...
- `stock-purchase-service`와 `strategy-execution-service`는 production-like profile에서 로컬 broker/Temporal endpoint, mock 주문 설정, 또는 로컬 `application-secret.properties` property source가 남아 있으면 startup validation으로 실패한다. root KIS wrapper도 production-like profile에서 로컬 secret property source를 기본 차단한다.
- root sketch app의 KIS secret key 이름은 `src/main/resources/application-secret.properties.example` 템플릿으로 문서화했고, classpath secret 파일 없이 런타임 환경변수/secret source로도 주입할 수 있다. production-like profile에서는 root wrapper, stock-purchase-service, strategy-execution-service 모두 로컬 `application-secret.properties` property source가 로드되면 startup validation으로 실패한다. 운영 재시작 절차는 `docs/operations/trading-runtime-runbook.md`에 정리했다.
