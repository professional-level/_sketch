...
- `stock-purchase-service`는 주문 제출 실패, `SUBMISSION_UNKNOWN` 지속, reconciliation 실패, 미매칭 broker execution을 `OperationalAlertPort`로 알리고, 기본 구현은 로그 기반 alert adapter와 Micrometer operational alert counter로 둔다.
...
- Slack/PagerDuty 같은 외부 alert sink, OpenTelemetry trace propagation, 운영 대시보드 UI는 아직 남아 있다.
...
17. 부분 완료: 운영 관측성을 추가한다. 주문 제출 실패, `SUBMISSION_UNKNOWN` 지속, reconciliation 실패, 미매칭 broker execution은 log 기반 alert port와 Micrometer counter로 노출하고, 라오어 전략별 T/현금/보유/평단 조회 API를 추가했다. 외부 alert sink, OpenTelemetry trace propagation, 대시보드 UI는 남아 있다.
