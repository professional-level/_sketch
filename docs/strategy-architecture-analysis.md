...
- KIS alias 기반 cash bucket 분리는 추가됐지만, 다통화/환율 반영과 계좌 현금 상태 이벤트 계약 확장은 아직 남아 있다.
...
- Risk guard cash checks prefer `orderableCashAmount` and fall back to the legacy `availableCashAmount` only for compatibility with older account snapshot providers. Strict multi-currency conversion, FX-rate normalization, and broader account cash/exposure modeling remain hardening work.
