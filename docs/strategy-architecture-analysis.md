...
- KIS alias 기반 cash bucket 분리와 risk guard 내부 기준 통화 환산은 추가됐다. FX source는 static 설정 또는 HTTP provider로 분리됐지만, KIS/은행권 전용 FX adapter와 계좌 현금 상태 이벤트 계약 확장은 아직 남아 있다.
...
- Risk guard cash checks prefer `orderableCashAmount` and fall back to the legacy `availableCashAmount` only for compatibility with older account snapshot providers. Order notional, pending buy notional, broker exposure, and orderable cash are normalized to the configured risk base currency through an `FxRatePort`; the default provider uses static `rates-to-base`, and an optional HTTP provider can fetch operator-managed live rates. KIS/은행권 전용 FX adapter and broader account cash/exposure modeling remain hardening work.
