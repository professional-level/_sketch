...
- Domestic and overseas status recovery treat KIS rejection reason aliases, including raw `rjct_rson_cd`/`RJCT_RSON_CD`, as a broker rejection signal even when only the rejection code is present. The domestic wrapper preserves that field in the daily execution order protobuf contract.
