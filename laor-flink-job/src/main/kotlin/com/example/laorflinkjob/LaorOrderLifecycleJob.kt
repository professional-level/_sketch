package com.example.laorflinkjob

import com.example.laorflinkjob.adapter.`in`.kafka.OrderCancelledEventDeserializer
import com.example.laorflinkjob.adapter.`in`.kafka.OrderFilledEventDeserializer
import com.example.laorflinkjob.adapter.`in`.kafka.OrderIntentCreatedEventDeserializer
import com.example.laorflinkjob.adapter.`in`.kafka.OrderPartiallyFilledEventDeserializer
import com.example.laorflinkjob.adapter.`in`.kafka.OrderRejectedEventDeserializer
import com.example.laorflinkjob.adapter.`in`.kafka.OrderSubmittedEventDeserializer
import com.example.laorflinkjob.adapter.out.kafka.LaorMilestoneKafkaRecordSerializer
import com.example.laorflinkjob.application.LaorOrderExecutionEvent
import com.example.laorflinkjob.application.LaorOrderLifecycleProcessor
import com.example.laorflinkjob.application.LaorOrderLifecycleProcessFunction
...
data class LaorOrderLifecycleJobConfig(
    ...
    fun kafkaSink(): KafkaSink<com.example.laorflinkjob.application.LaorMilestoneEnvelope> {
        return KafkaSink.builder<com.example.laorflinkjob.application.LaorMilestoneEnvelope>()
            .setBootstrapServers(bootstrapServers)
    ...
    companion object {
        fun fromEnvironment(env: Map<String, String> = System.getenv()): LaorOrderLifecycleJobConfig {
            return LaorOrderLifecycleJobConfig(
                ...
                groupId = env["LAOR_FLINK_GROUP_ID"] ?: "laor-flink-job",
